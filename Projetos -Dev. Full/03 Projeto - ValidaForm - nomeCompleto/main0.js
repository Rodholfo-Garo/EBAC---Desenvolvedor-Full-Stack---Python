...
form.addEventListener('submit', function(e){
    ...
    //mensagem de sucesso, o atributo inerhtml inseri um conteudo HTML
    if (formValido){
...
// EVENTO ADICIONADO AO CAMPO NOME COMPLETO - O evento keyup Confere em tempo real oq esta sendo digitado.
// Elevar nomeBeneficiario para o escobo global

nomeBenefi.addEventListener('keyup', function(e){
    console.log(e);
    formValido = validaNome(e.target.value);

    if (!formValido){
        nomeBenefi.classList.add('error');
        // nomeBenefi.style.border = '1px solid red'; estilo inline

        //mostra a mensagem de erro enquanto não for satisfeito
        document.querySelector('.error-message').style.display = 'block';
    }else{
        nomeBenefi.classList.remove('error');
        /* nomeBenefi.style="";//tira o contorno do input quando não estiver em focu */
